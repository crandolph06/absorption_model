...
def run_parallel_sweep():
    ...
    completed_batches = {
        ...
        if f.startswith('batch_4_') and f.endswith('.parquet')
    }

    if completed_batches:
        last_batch = max(completed_batches)
        last_file = os.path.join(OUTPUT_DIR, f"batch_4_{last_batch:04d}.parquet")
    ...
    with ProcessPoolExecutor() as executor:
        for result in executor.map(process_single_config, valid_configs, chunksize=2000):
            ...
            if len(buffer) >= CHUNK_SIZE:
                batch_index += 1
                batch_file = os.path.join(OUTPUT_DIR, f"batch_4_{batch_index:04d}.parquet")
    ...
    with open("SWEEP_4_COMPLETE.txt", "w") as f:
        f.write("Done")
    print(f"\n✅ Sweep 4 Complete. Total valid configs: {count}")

...
def run_parallel_sweep():
    ...
    with ProcessPoolExecutor() as executor:
        for result in executor.map(process_single_config, valid_configs, chunksize=2000):
            ...
            if len(buffer) >= CHUNK_SIZE:
                batch_index += 1
                batch_file = os.path.join(OUTPUT_DIR, f"batch_3A_{batch_index:04d}.parquet")
    ...
    with open("SWEEP_3A_COMPLETE.txt", "w") as f:
        f.write("Done")
    print(f"\n✅ Sweep 3A Complete. Total valid configs: {count}")

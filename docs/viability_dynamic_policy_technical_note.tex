...
candidates; every reported result is checked with direct physics. The primary
run evaluated 615 three-epoch policies over a 60-phase horizon. No feasible
policy was found. The best policy had $\phi=6.00173$ and required simultaneous
relaxation of the total-pilot window, WG RAP, and FL RAP constraints. A local
finite-difference diagnostic shows that the binding constraints respond to
different controls, and in some cases move in opposing directions. The result
does not prove global infeasibility, but it gives a concrete nearest miss and a
defensible basis for the next search or requirements trade.
\end{abstract}
...
\begin{itemize}
  ...
  structured heuristic seeds, GPR fitting, surrogate-guided candidate proposal,
  direct physics verification, and local finite-difference diagnostics.
  \item \texttt{src/viability/cli.py}: adds \texttt{dynamic-search} and
  \texttt{dynamic-diagnose} commands.
...
The primary search used the physics-backed configuration in
...
The primary run evaluated 519 initial schedules and 96 optimizer-proposed
schedules, for 615 direct physics evaluations total.

\section{Results}

No feasible policy was found in the tested three-epoch policy class. Table
\ref{tab:search-summary} gives the run summary.

\begin{table}[htbp]
\centering
\caption{Primary dynamic-policy search summary.}
...
Initial direct evaluations & 519 \\
Optimizer direct evaluations & 96 \\
Total direct evaluations & 615 \\
...
The best observed schedule was constant across all three epochs:
\begin{center}
\begin{tabular}{lr}
\toprule
Control & Value \\
...
UTE & 20.0 \\
PAA & 30 \\
Maximum manning percentage & 180.0 \\
FLUG quota per phase & 2 \\
IPUG quota per phase & 0 \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
Total-pilot window & 456 pilots \\
WG RAP shortfall & 6.00173 \\
FL RAP shortfall & 2.86928 \\
\bottomrule
\end{tabular}
\end{table}

The active constraint distribution over all direct evaluations was 325 FL RAP,
173 WG RAP, and 117 total-pilot window. The minimum observed value of each core
...
\midrule
$\phi$ & Epoch 3 retention rate & -1.778 \\
Total-pilot window & Epoch 2 retention rate & 90.91 \\
WG RAP & Epoch 3 retention rate & -1.778 \\
FL RAP & Epoch 2 retention rate & 7.701 \\
IP RAP & Epoch 2 retention rate & -2.318 \\
\bottomrule
\end{tabular}
\end{table}

Retention has the clearest local authority over the binding WG RAP miss. The
same neighborhood shows competing effects on FL RAP and the total-pilot window.
IPUG perturbations improve IP RAP locally but worsen the current objective and
FL RAP near this schedule. The diagnostic therefore points to a joint-feasibility
problem, not to a single constraint that is impossible to satisfy on its own.

\section{Limitations}

A 615-evaluation structured search does not prove global infeasibility. It
tests one policy class: three open-loop epochs with bounded controls and
surrogate-guided candidate proposal. A fully free per-phase schedule would have
many more degrees of freedom. That option was deliberately deferred to keep the
implementation interpretable and the evidence tractable.
...
\begin{enumerate}
  ...
  \item For additional search, refine around the best policy with asymmetric
  epoch patterns in the controls that showed local authority.
  \item For decision support, start the requirements trade study from the best
  observed miss: 456 pilots in the total-pilot window, 6.00173 WG RAP, and
  2.86928 FL RAP.
\end{enumerate}
...
interfaces. The primary result is negative: 615 direct physics evaluations
found no feasible policy in the three-epoch class. The run still produced a
clear nearest miss, quantified the required relaxations, and identified local
control authority around the binding constraints. The next step is either a
focused structured refinement near the best trajectory or a requirements trade
study anchored on that miss.
...
The local diagnostic was run with:
\begin{verbatim}
RUN=outputs/viability/dynamic_policy_search/run_3epoch_512_32768_096
venv/bin/python -m src.viability.cli dynamic-diagnose \
  --config outputs/viability/dynamic_policy_search/config_physics.yaml \
  --evaluations ${RUN}/all_evaluations.parquet \
  --output-dir ${RUN}/diagnostic \
  --epochs 3 \
...
\begin{itemize}
  \item 43 viability tests passed with \texttt{venv/bin/python -m unittest}.
...
\path{outputs/viability/dynamic_policy_search/run_3epoch_512_32768_096/diagnostic/dynamic_control_report.md}.

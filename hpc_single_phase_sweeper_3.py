...
OUTPUT_DIR = "outputs/single_phase/parquet/3"  
CHUNK_SIZE = 500000 

os.makedirs(OUTPUT_DIR, exist_ok=True)

def get_sweep_configs():
    ute_values = range(12, 15)
...
def run_parallel_sweep():
    ...
    with ProcessPoolExecutor() as executor:
        for result in executor.map(process_single_config, valid_configs, chunksize=2000):
            ...
            if len(buffer) >= CHUNK_SIZE:
                batch_index += 1
                batch_file = os.path.join(OUTPUT_DIR, f"batch_3_{batch_index:04d}.parquet")
    ...
    with open("SWEEP_3_COMPLETE.txt", "w") as f:
        f.write("Done")
    print(f"\n✅ Sweep 3 Complete. Total valid configs: {count}")
